async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        switch (true) {
            // Games
            case /^[1-9]$/.test(userMessage) || userMessage.toLowerCase() === 'surrender':
                await handleTicTacToeMove(sock, chatId, senderId, userMessage);
                break;

            // Economy Commands
            case userMessage.startsWith('.wallet'):
                const balance = economy.getWallet(senderId);
                await sock.sendMessage(chatId, { text: `💰 Your wallet: ${balance} coins`, ...channelInfo });
                break;

            case userMessage.startsWith('.daily'):
                const { reward, error } = economy.claimDaily(senderId);
                if (error) await sock.sendMessage(chatId, { text: `❌ ${error}`, ...channelInfo });
                else await sock.sendMessage(chatId, { text: `🎁 You claimed your daily reward: 💰${reward} coins!`, ...channelInfo });
                break;

            case userMessage.startsWith('.store'):
                let storeText = '🛒 Store:\n';
                for (let item in economy.store) storeText += `${item} - 💰${economy.store[item]}\n`;
                await sock.sendMessage(chatId, { text: storeText, ...channelInfo });
                break;

            case userMessage.startsWith('.buy'):
                const itemToBuy = userMessage.slice(5).trim();
                if (!itemToBuy) {
                    await sock.sendMessage(chatId, { text: '❌ Specify an item to buy!', ...channelInfo });
                    break;
                }
                const buyResult = economy.buyItem(senderId, itemToBuy);
                await sock.sendMessage(chatId, { text: buyResult, ...channelInfo });
                break;

            case userMessage.startsWith('.inventory'):
                const inv = economy.getInventory(senderId);
                await sock.sendMessage(chatId, { text: inv, ...channelInfo });
                break;
            ...
            case userMessage.startsWith('.slot'):
                const parts = userMessage.split(' ');
                const bet = parseInt(parts[1]);
                if (!bet) {
                    await sock.sendMessage(chatId, { text: '❌ Specify a bet amount!', ...channelInfo });
                    break;
                }
                const spinResult = economy.spinSlot(senderId, bet);
                if (spinResult.error) await sock.sendMessage(chatId, { text: `❌ ${spinResult.error}`, ...channelInfo });
                else await sock.sendMessage(chatId, { text: `🎰 You spun the slot! Winnings: 💰${spinResult.winnings}`, ...channelInfo });
                break;

            // Other commands here (e.g., .kick, .mute, .ban, .simage, etc.)
            default:
                // Load other commands dynamically or continue with your existing ladder
                break;
        }
